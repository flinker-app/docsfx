---
title: Load and save BCF files in Microsoft Teams
description: Learn how to load, review, and save BCF files in Microsoft Teams using the IFC Viewer for Microsoft Teams and SharePoint links.
keywords: BCF in Microsoft Teams, IFC Viewer for Microsoft Teams, load BCF file, save BCF file, SharePoint BCF workflow
canonical_url: https://docs.flinker.app/docs/update-bcf-files-in-microsoft-teams.html
---

# Load and save BCF files with Microsoft Teams

This guide explains how you can work with BCF files in Microsoft Teams.

## Overview

- **BCF files:** BCF (BIM Collaboration Format) files allow you to store and share building information.
- **SharePoint:** Your BCF files are stored on SharePoint, making them easy to share and collaborate on.

## Load a BCF file

1. **Open the IFC Viewer for Microsoft Teams:** Launch the IFC Viewer for Microsoft Teams.
2. **Enter your file link:**  
    - Paste the appropriate file link into the Teams tab settings.  
    - The IFC Viewer for Microsoft Teams will automatically load the BCF file stored on SharePoint using this link.
3. **Load the file:**  
    - Once the correct file link is set, IFC Viewer for Microsoft Teams will automatically retrieve its content.
    - A preview of the BCF file along with its topics and associated details will be displayed.

> [!NOTE]
> If the file is not found or cannot be loaded, verify that the file link in the Teams tab settings is correct and that you have the necessary SharePoint permissions.

## Save or update a BCF file

1. **Edit the file information:**  
    - Review and edit the topics and details within the loaded BCF file as needed.
2. **Save your changes:**  
    - Click the "Save to Teams/SharePoint" button.
    - IFC Viewer for Microsoft Teams will update the file directly on Teams/SharePoint.
    - A confirmation message appears once the file is updated.

> [!NOTE]
> Saving changes requires proper permissions on SharePoint.

## Troubleshooting

- **Permission issues:**  
  - Ensure you’re logged in with the correct account and have write access to SharePoint.
- **File loading and saving issues:**  
  - Check your internet connection.
  - Verify that the BCF file is in a supported format (.bcf).
