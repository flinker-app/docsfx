---
title: Report templates for the IFC Viewer Visual for Power BI
description: Review report template scenarios for using IFC Viewer Visual for Power BI in BIM, project controls, and facility management workflows.
keywords: IFC, Power BI, BIM, use cases, project management, data visualization, Microsoft 365, construction analytics, facility management
canonical_url: https://docs.flinker.app/docs/use-cases-for-ifc-visual-in-power-bi.html
---

# Report templates for the IFC Viewer Visual for Power BI

Use report templates to connect IFC model data with Power BI dashboards. Templates can support model review, cost reporting, schedule analysis, issue tracking, and facility management scenarios.
