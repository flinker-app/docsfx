...
# IFC to GIS portfolio map dashboard
...
2.  **Site and location:** Extracts `IfcSite` for geospatial coordinates and elevation.
...
## Recommended IFC specifications
...
> [!NOTE]
> If explicit coordinates are missing, the dashboard will attempt to use the Project Address (City, Country) for approximate location via Azure Maps geocoding.
...
### Projects not appearing on the map
| Symptom | Cause | Solution |
| :--- | :--- | :--- |
| Warning icon | IFC file has no coordinates or address | Re-export IFC with geolocation enabled |
...
> [!NOTE]
> Actual performance varies based on file complexity, machine specifications, and network speed (for SharePoint/network files). Test with your specific files to establish baseline refresh times.
...
## Add files and set up the files parameter in Power BI
...
### Step 4: Edit the parameter value
